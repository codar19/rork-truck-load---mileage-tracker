...
export default function ExpenseFormScreen() {
  ...
  const { category: categoryParam } = useLocalSearchParams<{ category: ExpenseCategory }>();
  const { user } = useAuth();
  const { addExpense } = useExpenses();
  const { loads } = useLoads();
  
  const category = categoryParam as ExpenseCategory;
  ...
  const handleTakePhoto = async () => {
    console.log('[ExpenseForm] Opening camera for receipt photo');
    
    if (!permission) {
      console.log('[ExpenseForm] Requesting camera permission');
      return;
    }

    if (!permission.granted) {
      console.log('[ExpenseForm] Camera permission not granted, requesting');
  ...
  const handleSubmit = () => {
    console.log('[ExpenseForm] Submit button pressed');
    console.log('[ExpenseForm] Current state:', {
      user: user?.id,
      category,
      amount,
      date,
    ...
    if (!user) {
      console.log('[ExpenseForm] Error: User not authenticated');
      Alert.alert('Error', 'User not authenticated');
      return;
    }

    if (!category) {
      console.log('[ExpenseForm] Error: No category provided');
      Alert.alert('Error', 'Invalid expense category');
      return;
    }

    if (!amount.trim() || isNaN(Number(amount)) || Number(amount) <= 0) {
      console.log('[ExpenseForm] Error: Invalid amount');
      Alert.alert('Error', 'Please enter a valid amount');
      return;
    }

    console.log('[ExpenseForm] Submitting expense:', category, Number(amount));
          ...
          <SafeAreaView style={styles.cameraOverlay} edges={['top', 'bottom']}>
            ...
            <View style={styles.cameraFooter}>
              <TouchableOpacity
                ...
                onPress={() => {
                  console.log('[ExpenseForm] Photo captured');
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <View style={styles.headerIconWrapper}>
            {getCategoryIcon(category, 20, '#f59e0b')}
          </View>
          <Text style={styles.headerTitle}>{categoryInfo?.label || 'Expense'}</Text>
        </View>
        <View style={{ width: 24 }} />
      </View>
      ...
      <View style={styles.footer}>
        ...
        >
          ...
          <Text style={styles.submitButtonText}>Add {categoryInfo?.label} Expense</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
